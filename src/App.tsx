    ...
    const handleAddDoctorNote = async (pId: string, note: string) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log("📦 Doctor note saved:", savedNote);
          ...
          console.error("❌ Failed to save doctor note");
        }
      } catch (error) {
    ...
    useEffect(() => {
      const interval = setInterval(() => {
        ...
        medicines.forEach(med => {
          ...
          if (med.stock < 5) {
            const alreadyAlerted = alerts.some(a => a.message.includes(med.name) && a.type === 'critical');
            if (!alreadyAlerted) {
        ...
        reminders.forEach(rem => {
          if (rem.time === currentTime && !rem.completed) {
            ...
            setReminders(prev => prev.map(r => r.id === rem.id ? { ...r, completed: true } : r));
          }
        });
      }, 60000);
